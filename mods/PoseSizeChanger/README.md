...
- **v2.0.0-alpha.2** -- DIAGNOSTIC RELEASE: Fixed Vector3 construction (field-by-field instead of constructor args), added LoadAppearance() call after SetVisualScale for aggressive refresh, added "Scale Test" diagnostic button that tests the full scaling pipeline (property write/read verification, chunkMask toggle to verify RefreshAppearance VFunc, field vs method consistency check). Run this test to diagnose why visual scaling may not be working.
