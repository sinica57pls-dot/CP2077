...
## Session 5 — Root Cause Analysis & Enhanced Diagnostics

**Date:** 2026-04-11 (continued)
**Trigger:** User reported: "Nothing we did in releases is working. Only the UI in CET showing the mod. But no visual changes and it doesn't work."

### Critical User Feedback

Across ALL releases (v1.0.0 through v2.0.0-alpha):
- The CET ImGui overlay renders correctly
- The Redscript system loads and registers callbacks
- The targeting, hotkeys, and diagnostics work
- **But the actual visual scaling has ZERO visual effect in-game**

This means every approach so far — direct field write (v1.0.3), @addField native (v1.0.4), RTTI property + RefreshAppearance (v2.0.0-alpha) — has failed to produce visible scaling on characters.

### Root Cause Analysis

After exhaustive investigation of the Codeware framework internals, here is the analysis:

#### The Fundamental Problem: visualScale Doesn't Exist Natively on Skinned Mesh Types

**Evidence:**

1. **entSkinnedMeshComponent imports** (`scripts/Base/Imports/entSkinnedMeshComponent.reds`): Lists 15 native fields — `mesh`, `meshAppearance`, `castShadows`, `castLocalShadows`, `acceptDismemberment`, `chunkMask`, `renderingPlaneAnimationParam`, `visibilityAnimationParam`, `order`, `isEnabled`, `LODMode`, `forcedLodDistance`, `overrideMeshNavigationImpact`, `navigationImpact`, `version`. **NO `visualScale`.**

2. **MeshComponent addons** (`scripts/Base/Addons/MeshComponent.reds:23`): `@addField(MeshComponent) public native let visualScale: Vector3;` — Codeware's author explicitly adds this because MeshComponent DOES have it in the C++ engine. The `native` keyword maps to the real C++ field.

3. **entDecalComponent imports** (`scripts/Base/Imports/entDecalComponent.reds:7`): `public native let visualScale: Vector3;` — in the IMPORTS (not addons), confirming the engine has it natively on decal components.

4. **Codeware's author deliberately does NOT expose visualScale on skinned types** — this is the strongest evidence that the field doesn't exist in the engine's C++ RTTI for those classes.

#### Why Our Approaches Failed

| Approach | Version | What Happens | Why No Visual Change |
|----------|---------|-------------|---------------------|
| Direct field write | v1.0.3 | Writes to Redscript extension field | Renderer reads from C++ native memory, not Redscript extensions |
| @addField native | v1.0.4 | `native` keyword tries to find C++ RTTI property; fails silently | If RTTI property doesn't exist, creates extension field in script memory |
| RTTI property write + RefreshAppearance | v2.0.0-alpha | `Red::GetPropertyPtr("visualScale")` finds the Redscript-added property | Writes to script-side memory; RefreshAppearance re-reads from C++ native memory (which has no visualScale for skinned types) |

#### Class Hierarchy Analysis

```
IVisualComponent (abstract base)
  ├── entDecalComponent              ← HAS visualScale natively
  ├── MeshComponent                  ← HAS visualScale (Codeware @addField native maps to it)
  │   └── HudMeshComponent
  └── entISkinTargetComponent        ← SIBLING of MeshComponent, NOT child
      ├── entSkinnedMeshComponent    ← NO visualScale in C++ RTTI
      │   ├── entGarmentSkinnedMeshComponent
      │   └── entCharacterCustomizationSkinnedMeshComponent
      ├── entMorphTargetSkinnedMeshComponent  ← NO visualScale in C++ RTTI
      └── entSkinnedClothComponent
```

Characters (Player V, NPCs) use **exclusively** skinned mesh components for their body, head, limbs, and clothing. They do NOT use MeshComponent. Since visualScale doesn't exist natively on skinned types, NO amount of writing to a visualScale property (real or extension) will affect rendering.

#### Potential Second Bug: Vector3 Construction

The mod used `new Vector3(factor, factor, factor)` throughout. Zero occurrences of this pattern exist in any Codeware script. In Redscript, native struct constructors may not support positional arguments — the `new` keyword may create a default-initialized (all zeros) struct while silently ignoring arguments.

**Fixed in v2.0.0-alpha.2** with field-by-field assignment:
```redscript
let v: Vector3;
v.X = factor;
v.Y = factor;
...
### Phase 16: Enhanced Diagnostics (v2.0.0-alpha.2)

Since we cannot test in-game, the best approach is to give the user tools to diagnose EXACTLY what's happening at each step.

#### Changes Made

**PoseSizeChangerSystem.reds:**

1. **Fixed Vector3 construction** — Replaced all `new Vector3(x, y, z)` with `MakeScaleVector(factor)` / `MakeDefaultVector()` helper functions using field-by-field assignment
2. **Added LoadAppearance() after SetVisualScale** — `LoadAppearance(false)` forces a full resource reload (VFunc 0x260) then RefreshAppearance (VFunc 0x280), which is more aggressive than RefreshAppearance alone
3. **Added RunScaleTest()** — Comprehensive diagnostic that tests the FULL pipeline:
   - **Test A**: Write/read visualScale on entSkinnedMeshComponent — verifies property write persistence
   - **Test B**: Write/read visualScale on entMorphTargetSkinnedMeshComponent
   - **Test C**: Write/read visualScale on MeshComponent (if any found)
   - **Test D**: chunkMask toggle + RefreshAppearance on skinned mesh — verifies VFunc 0x280 works. If the body briefly flickers, RefreshAppearance IS functional on skinned types (proving the issue is visualScale-specific, not RefreshAppearance being broken)
   - **Test E**: Direct field read/write consistency — Tests whether `skinned.visualScale = x` and `skinned.GetVisualScale()` read/write the SAME memory location. If they read different values, it proves @addField and Red::GetPropertyPtr resolve to different locations

**init.lua:**
4. **Added "Run Scale Test" button** — Calls RunScaleTest() and displays verbose results with color-coded output
5. **Refactored result display** — Extracted helper functions (ReadResultArray, DrawResultLine) to reduce duplication

#### What The Scale Test Will Tell Us

| Result | Meaning | Next Step |
|--------|---------|-----------|
| Test A shows "Write verified" | Property write/read works (but may be to extension field) | Confirms issue is renderer-side, not write-side |
| Test A shows "returned zeros" | GetPropertyPtr returns null, property doesn't exist | Extension field is not created or `native` failed |
| Test D causes visual flicker | RefreshAppearance VFunc works on skinned types | Confirms issue is specifically visualScale |
| Test D has no visual effect | RefreshAppearance VFunc may not work on skinned types | VFunc 0x280 may be mapped to a different function on SkinnedMeshComponent vtable |
| Test E shows "DIFFERENT locations" | @addField field and Red::GetPropertyPtr resolve differently | Confirms two separate fields exist — one script-side, one RTTI-side |

#### Alternative Approaches for Future Investigation

If the Scale Test confirms visualScale doesn't work on skinned meshes, these alternative approaches should be investigated:

1. **WorldNodeInstance.Scale** (OffsetPtr<0x50, Vector3>) — Entity-level scale in world. Problem: no Entity→WorldNodeInstance path; only reverse link. Would need streaming sector iteration to find the entity's node.

2. **Entity.Reassemble with appearance overrides** — `appearancePartComponentOverrides` has `visualScale`. If we hook Entity_Reassemble and inject our scale into the component creation pipeline, the engine's own code would apply it.

3. **Root bone transform scaling** — Modify the animation skeleton's root bone transform to include scale. Characters use `entAnimatedComponent` for skeletal animation; scaling the root bone would propagate to all vertices.

4. **Render proxy world matrix modification** — Hook the render submission and modify the world matrix to include scale. Would require finding the render submission function.

5. **MorphTarget system** — `Raw::MorphTargetManager_ApplyMorphTarget` exists and can deform body regions. Not true uniform scaling but could approximate it.

6. **IComponent.Toggle** (addr 3370191745) — Full component disable/enable cycle instead of RefreshAppearance. More aggressive; forces complete render state teardown and rebuild.

### Files Changed

```
MOD: mods/PoseSizeChanger/.../PoseSizeChangerSystem.reds  -- Vector3 fix, LoadAppearance, RunScaleTest
MOD: mods/PoseSizeChanger/.../init.lua                    -- Scale Test UI, refactored display
MOD: CLog.md                                              -- Root cause analysis (this section)
```
...
| Release | Tag | Summary |
|---------|-----|---------|
| v2.0.0-alpha.2 | `pose-size-changer-v2.0.0-alpha.2` | **Diagnostic release**: Vector3 fix, multi-refresh, Scale Test |

---

*Session 5 root cause analysis & diagnostics generated by Claude Code on 2026-04-11*
